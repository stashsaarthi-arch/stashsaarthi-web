...
export function verifyTamperHologramCodeFormat(code: string): {
  ...
  if (clean.length < 6) {
    return {
      ...
      error: "Hologram code must be at least 6 characters.",
    };
  }

  // Accepts standard StashSaarthi hologram patterns
  const pattern =
    /^(STASH-HOL-[A-Z0-9]{4,8}|SS-HOLOTAPE-[A-Z0-9]{4,10}|HOL-[A-Z0-9]{4,8}|SS-SEAL-[A-Z0-9]{4,8}|[A-Z0-9]{8,16})$/;
  if (!pattern.test(clean)) {
    return {
      isValid: false,
      normalizedCode: clean,
      error:
        "Invalid hologram format. Expected format like STASH-HOL-XXXXXX or SS-HOLOTAPE-2026-XXXX.",
    };
  }
...
export function generateHologramSecuritySealHash(
  ...
  const part2 = Math.abs(hash * 37)
    .toString(16)
    .padStart(8, "0");
  const part3 = Math.abs(hash * 109)
    .toString(16)
    .padStart(8, "0");
...
export function getTamperHologramRecord(codeOrBookingId: string): TamperHologramRecord | null {
  ...
  return (
    records.find(
      (r) =>
        r.hologramCode.toUpperCase() === search ||
        r.bookingId.toUpperCase() === search ||
  ...
  const newRecord: TamperHologramRecord = {
    id:
      existingIndex !== -1
        ...
        : `HOLO-REC-${Math.floor(1000 + Math.random() * 9000)}`,

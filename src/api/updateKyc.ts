...
export async function handleUpdateKyc(request: Request): Promise<Response> {
  ...
  try {
    ...
    const db = getAdminDb();
    if (!auth || !db) {
      return new Response(JSON.stringify({ success: false, error: 'Backend authentication service is temporarily unconfigured.' }), { status: 503 });
    }

    if (!process.env['CLOUDINARY_CLOUD_NAME'] || !process.env['CLOUDINARY_API_KEY'] || !process.env['CLOUDINARY_API_SECRET']) {
      return new Response(JSON.stringify({ success: false, error: 'Cloudinary credentials unconfigured on server.' }), { status: 503 });
    }

    const authHeader = request.headers.get('Authorization');
    if (!authHeader?.startsWith('Bearer ')) {
      return new Response(JSON.stringify({ success: false, error: 'Unauthorized: Missing or invalid token format' }), { status: 401 });
    }
    
    const idToken = authHeader.split('Bearer ')[1];
    if (!idToken) {
      return new Response(JSON.stringify({ success: false, error: 'Unauthorized: Invalid token structure' }), { status: 401 });
    }
    ...
    try {
      ...
      return new Response(JSON.stringify({ success: false, error: 'Unauthorized: Token verification failed' }), { status: 401 });
    }

...
import { ArrowRight, BadgeCheck, Boxes, IndianRupee, MapPin, ShieldCheck, Lock, Banknote, Clock, Phone } from "lucide-react";
  ...
  return (
    <section id="top" className="relative overflow-hidden pb-16 pt-28 sm:pb-24 md:pt-44">
      ...
      <div className="relative mx-auto max-w-6xl px-4 text-center">
        <AnimatedContent distance={60} direction="vertical" duration={0.9} ease="power3.out" delay={0.1}>
          <span className="glass mx-auto inline-flex max-w-full items-center gap-2 rounded-full px-3 py-2 text-[9px] font-semibold uppercase leading-tight tracking-[0.14em] sm:px-4 sm:text-[11px] sm:tracking-[0.18em]" style={{ color: student ? 'var(--destructive)' : 'var(--muted-foreground)' }}>
            {student ? (
          ...
          <div className="mt-8 flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:items-center">
            ...
            >
              <span className="truncate">
                {student ? t.hero.student.cta : t.hero.host.cta}
              </span>
              <ArrowRight className="shrink-0 transition-transform group-hover:translate-x-1 ml-2" />
            </Button>
            {student ? (
              <Button data-magnetic variant="frost" size="xl" asChild className="w-full sm:w-auto px-6 py-6">
                <a href="#ecosystem">
                  {t.hero.student.secondaryCta}
                </a>
              </Button>
            ) : (
              <Button data-magnetic variant="outline" size="xl" asChild className="w-full sm:w-auto px-6 py-6 border-[#25D366]/50 bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20 hover:text-[#25D366]">
                <a 
          ...
          <div className="glass grid grid-cols-2 gap-px overflow-hidden rounded-3xl md:grid-cols-4">
            {STATS.map((s, i) => {
              ...
              return (
                ...
                >
                  <Icon className={`mx-auto mb-2 h-4 w-4 ${student ? "text-cyan" : "text-amber-400"}`} />
                  <AnimatedStat value={s.value} className="text-xl font-extrabold tracking-tight sm:text-2xl" style={{ transform: "translateZ(20px)", color: student ? "" : "#FDE68A" }} />
                  <div className="mt-1 text-[10px] uppercase tracking-wider text-muted-foreground sm:text-[11px] leading-tight" style={{ transform: "translateZ(30px)" }}>
                    {s.label}
                  </div>
                </AnimatedContent>
              );
        ...
        >
          <div className={`relative rounded-2xl border ${student ? "border-cyan-500/20 bg-cyan-950/10" : "border-amber-500/20 bg-amber-950/10"} px-4 py-3 backdrop-blur-md overflow-hidden`}>
            ...
            <div className={`absolute -inset-1 bg-gradient-to-r ${student ? "from-cyan-500/0 via-cyan-500/10 to-cyan-500/0" : "from-amber-500/0 via-amber-500/10 to-amber-500/0"} opacity-50 blur-xl animate-pulse`} />
            ...
            <p className="relative z-10 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-[11px] sm:text-xs font-semibold tracking-wider text-muted-foreground uppercase">
              <span className={`flex items-center ${student ? "text-amber-400" : "text-amber-300"}`}>

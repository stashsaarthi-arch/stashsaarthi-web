...
export function ReferralPill({ userType = "student" }: { userType?: "student" | "host" }) {
  ...
  return (
    <div className={`w-full rounded-2xl border ${userType === "host" ? "border-amber-500/30 bg-amber-950/20" : "border-emerald-500/30 bg-emerald-950/20"} p-4 backdrop-blur-md transition-all mt-4`}>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        
        {/* Left Side: Icon & Info */}
        <div className="flex items-center gap-3">
          <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border ${userType === "host" ? "bg-amber-500/20 text-amber-400 border-amber-500/30" : "bg-emerald-500/20 text-emerald-400 border-emerald-500/30"}`}>
          ...
          <div>
            <h4 className={`text-sm font-semibold ${userType === "host" ? "text-amber-300" : "text-emerald-300"}`}>
              {userType === "host" 
                ? (isHi ? "साथी होस्ट को रेफर करें" : "Refer a Fellow Host") 
                : (isHi ? "रेफर करें और 1 महीना मुफ्त पाएं" : "Refer & Earn 1 Month FREE")}
            </h4>
            <p className="text-xs text-slate-400">
              {userType === "host" 
                ? (isHi ? "पहले कमरे के ऑनबोर्ड होने पर ₹500 रेफरल बोनस पाएं।" : "Earn ₹500 referral bonus on first room onboarded.") 
                : (isHi ? "₹300 स्टैशक्रेडिट पाने के लिए दोस्तों के साथ साझा करें।" : "Share with hostel friends to unlock ₹300 StashCredit.")}
            </p>
          </div>
        </div>
        ...
        <div className="flex w-full sm:w-auto items-center justify-between sm:justify-end gap-2 shrink-0 flex-wrap">
          ...
          <button
            ...
            aria-label={isHi ? "व्हाट्सएप पर आमंत्रण शेयर करें" : "Share referral invite on WhatsApp"}
          >

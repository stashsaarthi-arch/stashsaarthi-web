...
export const ReverseLogisticsModal: React.FC<ReverseLogisticsModalProps> = ({
    ...
    if (isOpen) {
      ...
      if (initialBookingId) {
        ...
        const existing = all.find(
          (r) => r.bookingId === initialBookingId.toUpperCase() && r.status === "AUTHORIZED",
        );
        if (existing) {
  ...
  const handleCreateAuthorization = (e: React.FormEvent) => {
    ...
    if (!proxyName.trim() || !proxyPhone.trim() || !proxyGovtIdLast4.trim()) {
      alert(
        isHi
          ? "कृपया मित्र का नाम, मोबाइल नंबर और आईडी के अंतिम 4 अंक भरें।"
          : "Please enter proxy name, phone number, and last 4 digits of ID.",
      );
      return;
    }
  ...
  const handleRevoke = (id: string) => {
    if (
      confirm(
        isHi
          ? "क्या आप इस प्रॉक्सी अधिकृतीकरण को रद्द करना चाहते हैं?"
          : "Revoke this proxy handover authorization?",
      )
    ) {
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-xl font-bold text-white tracking-wide">
                  {isHi
                    ? "रिवर्स लॉजिस्टिक्स एवं प्रॉक्सी-हैंडओवर"
                    : "Reverse Logistics & Proxy-Handover Pass"}
                </h3>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                {isHi
                  ? "सुरक्षित 2FA मित्र सामान वितरण एवं सत्यापन प्रणाली"
                  : "Secure 2FA Proxy Handover Protocol for Out-of-City Students"}
              </p>
            </div>
          </div>
          <button
              ...
              <div className="lg:col-span-7 space-y-4">
                <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800">
                  <h4 className="text-sm font-semibold text-emerald-400 mb-1 flex items-center gap-2">
                    ...
                    {isHi
                      ? "मित्र / प्रॉक्सी अधिकृतीकरण विवरण"
                      : "Proxy Friend Authorization Details"}
                  </h4>
                  <p className="text-xs text-slate-400 mb-4">
                    {isHi
                      ? "छुट्टियां खत्म होने पर यदि आप कानपुर नहीं आ पा रहे हैं, तो अपने मित्र को सामान रिसीव करने का अधिकार दें।"
                      : "Authorize a trusted friend to collect your box if you cannot return to campus at break end."}
                  </p>
                  ...
                  <form onSubmit={handleCreateAuthorization} className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-xs font-medium text-slate-300 block mb-1">
                          {isHi
                            ? "मित्र की आईडी (आधार/कॉलेज) अंतिम 4 अंक"
                            : "Proxy Govt/College ID Last 4 Digits"}
                        </label>
                        <input
                          type="text"
                          maxLength={4}
                    ...
                    >
                      ...
                      {isHi
                        ? "2FA प्रॉक्सी डिजिटल पास जनरेट करें"
                        : "Generate 2FA Proxy Authorization Pass"}
                    </button>
                  </form>
                </div>
              </div>
                ...
                {createdPass ? (
                    ...
                    <div className="space-y-3 border-b border-slate-800 pb-3 mb-3">
                      <div>
                        <div className="text-[10px] text-slate-400 uppercase tracking-widest">
                          {isHi ? "बुकिंग रेफरेंस" : "Booking Reference"}
                        </div>
                        <div className="text-lg font-extrabold text-white font-mono">
                          {createdPass.bookingId}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div>
                          <span className="text-slate-400 block">
                            {isHi ? "अधिकृत मित्र:" : "Proxy Friend:"}
                          </span>
                          <span className="text-emerald-300 font-semibold">
                            {createdPass.proxyName}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-950/80 p-3 rounded-xl border border-slate-800 space-y-3">
                      <div className="text-center">
                        <div className="text-[10px] text-slate-400 uppercase tracking-widest mb-1">
                          {isHi ? "6-अंकीय सत्यापन पिन" : "6-Digit Verification PIN"}
                        </div>
                      ...
                      <div className="text-center">
                        <div className="text-[10px] text-slate-400 uppercase tracking-widest">
                          {isHi ? "पास कोड" : "Auth Code"}
                        </div>
                        <div className="text-xs font-mono font-bold text-slate-300">
                          {createdPass.authCode}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="p-8 rounded-2xl bg-slate-900/40 border border-slate-800 border-dashed text-center flex flex-col items-center justify-center min-h-[360px]">
                    ...
                    <p className="text-xs text-slate-500 max-w-xs">
                      {isHi
                        ? "बाएं फॉर्म में मित्र का विवरण भरकर 2FA सत्यापन पास बनाएं।"
                        : "Fill out the proxy authorization form on the left to issue a secure digital handover pass."}
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
                  ...
                  <div>
                    <h4 className="text-base font-bold text-white">
                      {isHi
                        ? "कैंपस रनर प्रॉक्सी सत्यापन टर्मिनल"
                        : "Campus Runner Proxy Verification Terminal"}
                    </h4>
                    <p className="text-xs text-slate-400">
                      {isHi
                        ? "सामान हैंडओवर से पूर्व विद्यार्थी के मित्र का 6-अंकीय पिन या पास कोड सत्यापित करें।"
                        : "Verify 6-digit PIN or authorization code before handing over luggage to designated friend."}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                    <div className="sm:col-span-8">
                      <label className="text-xs font-medium text-slate-300 block mb-1">
                        {isHi
                          ? "6-अंकीय पिन या ऑथ कोड दर्ज करें"
                          : "Enter 6-Digit PIN or Auth Code"}
                      </label>
                      <input
                        type="text"
                    ...
                    {verifyResult.record && (
                      <div className="bg-slate-950/60 p-3 rounded-lg border border-slate-800 space-y-1 font-mono text-[11px] text-slate-300 mt-2">
                        <div>
                          ...
                          {verifyResult.record.bookingId}
                        </div>
                        <div>
                          <span className="text-slate-500">Owner Student:</span>{" "}
                          {verifyResult.record.studentName} ({verifyResult.record.studentPhone})
                        </div>
                        <div>
                          <span className="text-slate-500">Authorized Proxy:</span>{" "}
                          {verifyResult.record.proxyName} ({verifyResult.record.proxyPhone})
                        </div>
                        <div>
                          <span className="text-slate-500">Proxy ID Verification:</span> Last 4
                          Digits ({verifyResult.record.proxyGovtIdLast4})
                        </div>
                        <div>
                          <span className="text-slate-500">Pickup Location:</span>{" "}
                          {verifyResult.record.deliveryNode}
                        </div>
                        <div>
                          <span className="text-slate-500">Status:</span>{" "}
                          <span className="font-bold text-white">{verifyResult.record.status}</span>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs font-semibold text-slate-300 px-1">
                  <span>
                    {isHi
                      ? "हाल के प्रॉक्सी अधिकृतीकरण रिकॉर्ड्स"
                      : "Recent Proxy Authorization Log Registry"}
                  </span>
                ...
                <div className="bg-slate-950 border border-slate-800 rounded-xl overflow-hidden text-xs">
                  ...
                  <div className="divide-y divide-slate-850 max-h-48 overflow-y-auto">
                    {records.map((r) => (
                      <div
                        key={r.id}
                        className="grid grid-cols-12 gap-2 p-2.5 items-center hover:bg-slate-900/50"
                      >
              ...
              <div className="p-4 rounded-xl bg-amber-950/30 border border-amber-500/30 text-amber-300 text-xs flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-bold text-sm text-amber-200">
                    {isHi
                      ? "स्टैशसारथी रिवर्स लॉजिस्टिक्स सुरक्षा प्रोटोकॉल"
                      : "StashSaarthi Reverse Logistics Security Charter"}
                  </h4>
                  <p className="mt-0.5 text-amber-300/80">
                    {isHi
                      ? "विद्यार्थी की अनुपस्थिति में अनधिकृत व्यक्ति को सामान सौंपना strictly प्रतिबंधित है।"
                      : "Guarantees zero unauthorized deliveries and 100% liability verification when students cannot return to campus."}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1.5">
                  ...
                  <p className="text-slate-400">
                    Only the registered booking student can generate the single-use 6-digit PIN and
                    authorization code from their authenticated portal.
                  </p>
                </div>
                ...
                <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1.5">
                  ...
                  <p className="text-slate-400">
                    Campus runner verifies the proxy friend’s physical Aadhaar / College ID card
                    last 4 digits against the recorded authorization before release.
                  </p>
                </div>
                ...
                <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1.5">
                  ...
                  <p className="text-slate-400">
                    Once released, the proxy token status shifts to CLAIMED and cannot be reused or
                ...
                <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1.5">
                  ...
                  <p className="text-slate-400">
                    An automated SMS and WhatsApp confirmation is dispatched to the original student
                    owner the moment the runner hands over the box.
                  </p>
                </div>
              </div>

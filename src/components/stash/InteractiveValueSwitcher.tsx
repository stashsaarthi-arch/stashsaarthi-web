...
import { AlertTriangle, ShieldCheck, ArrowRight, Sparkles, IndianRupee, CheckCircle2, XCircle, RefreshCw, SlidersHorizontal } from "lucide-react";
import { Button } from "@/components/ui/button";
import { playPop } from "@/lib/audio";
import { useLanguage } from "@/context/LanguageContext";
import { DeadRentSavingsSlider } from "@/components/ui/DeadRentSavingsSlider";
...
type ViewMode = "contrast" | "slider" | "dead_rent" | "saarthi_stash";
    ...
    <div className="w-full max-w-4xl mx-auto my-4 text-left">
      <div className="relative overflow-hidden rounded-2xl border border-emerald-500/30 bg-[#0A0D0F]/90 p-4 sm:p-6 backdrop-blur-2xl shadow-[0_0_50px_rgba(16,185,129,0.15)]">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
          ...
          <div className="inline-flex flex-wrap rounded-full border border-white/10 bg-black/60 p-1 backdrop-blur-md self-stretch sm:self-auto">
            <button
              type="button"
            ...
            <button
              ...
              className={`flex-1 sm:flex-initial rounded-full px-3 py-1 text-xs font-bold transition-all ${
                viewMode === "slider"
                  ? "bg-gradient-to-r from-emerald-400 via-teal-300 to-cyan-400 text-black shadow-md font-extrabold"
                  : "text-muted-foreground hover:text-white"
              }`}
            >
              {isHi ? "🎚️ लाइव ड्रैग कैलकुलेटर" : "🎚️ Live Drag Slider"}
            </button>
            <button
              type="button"

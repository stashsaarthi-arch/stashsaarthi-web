          ...
          {activeTab === 'generator' && (
            <div className="space-y-6">
              ...
              <div className="p-3.5 bg-amber-500/10 border border-amber-500/30 rounded-xl flex items-center gap-3 text-amber-300 text-xs">
                ...
                <div>
                  <strong className="block text-amber-200 font-semibold">
                    {isHi ? 'सेलुलर डेटा डिस्कनेक्टेड' : 'Cellular Data Disconnected (2G / No Network Zone)'}
                  </strong>
                  {isHi 
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-xs font-mono text-slate-400 mb-1">VERIFICATION OTP</label>
                  <input
                    type="text"
                    value={otp}
                ...
                <div>
                  <label className="block text-xs font-mono text-slate-400 mb-1">RUNNER AGENT ID</label>
                  <input
                    type="text"
                    value={runnerId}
                ...
                <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-5 space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-mono text-emerald-300 font-bold">HMAC-SHA256 ENCRYPTED PAYLOAD</span>
                    </div>
                    <span className="text-[10px] font-mono text-slate-400">GATEWAY: +91 9369454350</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                    ...
                    >
                      ...
                      {isHi ? '📱 एसएमएस ऐप में खोलें (smsto:)' : '📱 Open SMS App (Send to +91 9369454350)'}
                    </a>
                    ...
                    >
                      ...
                      {copied ? (isHi ? 'कॉपी किया गया!' : 'Copied Payload!') : (isHi ? 'पेलोड कॉपी करें' : 'Copy SMS Payload')}
                    </button>
                  </div>
                  ...
                  <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-xs font-semibold text-white block">Offline Optical Pass</span>
                        <span className="text-[10px] text-slate-400 block">Runner can also present this QR to Host scanner</span>
                      </div>
                    </div>
            ...
            <div className="space-y-6">
              <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 space-y-3">
                <label className="block text-xs font-semibold text-slate-300">
                  {isHi ? 'प्राप्त एन्क्रिप्टेड एसएमएस पेलोड यहाँ पेस्ट करें:' : 'Paste Received Encrypted SMS Payload Here:'}
                </label>
                <textarea
                  rows={3}
                  value={rawSmsInput}
                ...
                <div className="flex items-center justify-between pt-1">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => handlePresetFill(payload?.formattedSmsText || 'STASH-SMS-OTP|STASH-KNP-9821|8492|RUNNER-KAKADEO-01|' + Date.now() + '|A84B-99C1')}
                      className="text-[10px] font-mono text-cyan-400 underline hover:text-cyan-300"
                    >
                      Valid Active Sample
                    </button>
                    <button
                      onClick={() => handlePresetFill('STASH-SMS-OTP|STASH-KNP-9821|8492|RUNNER-TAMPERED|1770000000000|INVALID-SIG')}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-sm">
                        {verifyResult.success ? 'OTP Verification Successful' : 'Verification Failed'}
                      </h4>
                  ...
                  {verifyResult.success && (
                    <div className="mt-4 pt-4 border-t border-emerald-500/20 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs font-mono">
                      ...
                      <div>
                        ...
                        <span className="text-amber-300 font-bold">{verifyResult.verificationCode}</span>
                      </div>
                    </div>
                  )}
                </div>
          ...
          {activeTab === 'logs' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                ...
                <span className="text-xs text-slate-400 font-mono">{logs.length} Recorded Logs</span>
              </div>

              <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                {logs.map((log) => (
                  <div key={log.id} className="p-3 bg-slate-900/80 border border-slate-800 rounded-lg flex items-center justify-between text-xs font-mono">
                    <div>
                      <div className="flex items-center gap-2">
              ...
              <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-2 text-xs text-slate-400">
                ...
                <ul className="list-disc list-inside space-y-1 text-[11px] text-slate-400">
                  <li>HMAC-SHA256 salted signature prevents payload alteration or replay attacks.</li>
                  <li>30-minute timestamp window prevents stale OTP reuse across campus nodes.</li>
                  <li>Deep-linked <code className="text-emerald-300 font-mono">smsto:</code> format enables single-tap dispatch without cellular data connection.</li>
                </ul>
              </div>
            </div>
          )}

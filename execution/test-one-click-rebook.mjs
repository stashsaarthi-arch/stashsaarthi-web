...
if (!fs.existsSync(dashboardPath)) {
  ...
  if (
    !dashboardContent.includes("handleRebook") ||
    !dashboardContent.includes("handleReorderMeal")
  ) {
  ...
  if (!dashboardContent.includes("stashsaarthi:open-booking")) {
    ...
    console.log(
      "✅ Custom event 'stashsaarthi:open-booking' dispatch confirmed in MyBookingsDashboard.tsx",
    );
  }
}
...
if (!fs.existsSync(drawerPath)) {
  ...
  if (
    !drawerContent.includes("handleRenew") ||
    !drawerContent.includes("1-Tap Renew Storage Slot")
  ) {

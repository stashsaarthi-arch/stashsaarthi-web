...
assert.ok(engineContent.includes('STASHSAARTHI-CSO-HOLOGRAM-LOCK-2026'), 'Must use CSO cryptographic lock salt');
...
assert.ok(modalContent.includes('1. Link Hologram Tape') || modalContent.includes('Hologram Tape'), 'Modal must include link tab');
assert.ok(modalContent.includes('2. Hologram Audit Registry') || modalContent.includes('Audit Registry'), 'Modal must include audit registry tab');
assert.ok(modalContent.includes('3. Security Specifications') || modalContent.includes('CSO'), 'Modal must include charter tab');

assert.ok(adminContent.includes('TamperHologramProtocolModal'), 'admin.tsx must import TamperHologramProtocolModal');
assert.ok(adminContent.includes('isHologramModalOpen'), 'admin.tsx must maintain isHologramModalOpen state');

...
assert(sqlContent.includes('purge_inactive_student_data_18_months'), 'RPC function purge_inactive_student_data_18_months must be defined');
assert(sqlContent.includes('Anonymized Data Principal'), 'PII in financial records older than 18 months must be anonymized');
...
assert(engineContent.includes('INACTIVE_DAYS_THRESHOLD = 547'), 'Engine must set 547 days (18 months) threshold');
assert(engineContent.includes('auditInactiveStudentData'), 'auditInactiveStudentData function must exist');
assert(engineContent.includes('executeAutoPurge18Months'), 'executeAutoPurge18Months function must exist');
assert(engineContent.includes('initAutoDataRetentionPurge'), 'initAutoDataRetentionPurge function must exist');
...
assert(privacyContent.includes('18-Month Data Retention'), 'Privacy page must highlight 18-month data retention policy');

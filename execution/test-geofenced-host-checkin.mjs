...
assert(
  geoContent.includes("MAX_GEOFENCE_RADIUS_METERS = 50"),
  "Enforces MAX_GEOFENCE_RADIUS_METERS = 50",
);
assert(
  geoContent.includes("calculateDistanceMeters"),
  "Exports calculateDistanceMeters Haversine function",
);
assert(geoContent.includes("verifyGeoFenceLocation"), "Exports verifyGeoFenceLocation function");
assert(
  geoContent.includes("PRESET_CAMPUS_HOST_NODES"),
  "Exports PRESET_CAMPUS_HOST_NODES with Kanpur campus coords",
);

// 3. Check hostVerificationEngine content tokens
const hostContent = fs.readFileSync(hostEnginePath, "utf-8");
assert(
  hostContent.includes("geoFencePassed"),
  "HostStashVerification includes geoFencePassed field",
);
assert(
  hostContent.includes("verifyGeoFenceLocation"),
  "validateIntakeChecklist calls verifyGeoFenceLocation",
);
assert(
  hostContent.includes("Geo-Fence Lock: Check-in locked!"),
  "Includes geo-fence lock error message",
);

// 4. Check HostStashVerificationModal UI content tokens
const modalContent = fs.readFileSync(modalPath, "utf-8");
assert(
  modalContent.includes("Geo-Fenced Host Check-in Lock"),
  "Modal renders Geo-Fenced Host Check-in Lock title",
);
...
// Test 12m away -> PASS
const dist12m = calculateDistanceMeters(
  kakadeoLat,
  kakadeoLng,
  kakadeoLat + 0.0001,
  kakadeoLng + 0.00005,
...
const dist185m = calculateDistanceMeters(
  kakadeoLat,
  kakadeoLng,
  kakadeoLat + 0.0015,
  kakadeoLng + 0.0012,

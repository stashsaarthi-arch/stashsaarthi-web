...
function runVerification() {
  ...
  console.log(`   ✅ Defined ${STRESS_PROFILES.length} stress profiles (Dim Lighting, Crumpled Tape, etc.)`);
  ...
  console.log(`   ✅ Contrast Boost: ${preprocessed.enhancedContrast}x | Normalized Lux: ${preprocessed.normalizedBrightness} lx`);
  ...
  if (!singleRes.success || singleRes.confidenceScore < 80.0) {
    throw new Error(`❌ FAIL: Single decode simulation failed with score ${singleRes.confidenceScore}%`);
  }
  console.log(`   ✅ Single Decode Success: ${singleRes.barcode} (Confidence: ${singleRes.confidenceScore}%, Time: ${singleRes.scanTimeMs}ms)`);
  ...
  if (!summary.passTarget95Percent || summary.decodeRatePercent < 95.0) {
    throw new Error(`❌ FAIL: Decode rate ${summary.decodeRatePercent}% fell below 95.0% SLA target.`);
  }

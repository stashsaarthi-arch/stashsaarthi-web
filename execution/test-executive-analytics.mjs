...
assert.ok(
  fs.existsSync(executiveDashboardPath),
  "src/components/stash/ExecutiveAnalyticsDashboard.tsx must exist",
);
...
assert.ok(
  ceoAnalyticsContent.includes("calculateExecutiveMetrics"),
  "Must export calculateExecutiveMetrics",
);
assert.ok(
  ceoAnalyticsContent.includes("exportExecutiveAnalyticsJson"),
  "Must export exportExecutiveAnalyticsJson",
);
...
assert.ok(
  ceoAnalyticsContent.includes("activeTokensInCirculation"),
  "Must compute active token circulation",
);
assert.ok(
  ceoAnalyticsContent.includes("platformNetMarginPercent"),
  "Must define platform net margin",
);
...
assert.ok(
  dashboardContent.includes("Active Token Circulation"),
  "UI must render Token Circulation",
);
assert.ok(
  dashboardContent.includes("Interactive CAC / LTV & Payback Simulator"),
  "UI must render CAC/LTV Simulator",
);
...
assert.ok(
  adminContent.includes("ExecutiveAnalyticsDashboard"),
  "Admin page must import & render ExecutiveAnalyticsDashboard",
);
assert.ok(
  adminContent.includes('id: "executive"'),
  "Admin page must include executive analytics tab",
);

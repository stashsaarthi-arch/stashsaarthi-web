...
const emptyStatePath = path.join(process.cwd(), 'src', 'components', 'stash', 'CampusEmptyStateDelight.tsx');
...
if (!fs.existsSync(emptyStatePath)) {
  ...
  if (!content.includes("WELCOME50") || !content.includes("₹50") || !content.includes("stashsaarthi:open-booking")) {
    console.error("❌ Promo code WELCOME50, ₹50 discount card, or booking trigger missing in CampusEmptyStateDelight.tsx");
    passed = false;
  } else {
    console.log("✅ CampusEmptyStateDelight component confirmed with WELCOME50 promo code & ₹50 discount card!");
  }
}

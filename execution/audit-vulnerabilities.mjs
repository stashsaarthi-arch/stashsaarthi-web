...
const deps = auditData.metadata?.dependencies || {
  prod: 0, dev: 0, optional: 0, peer: 0, total: 0
};
...
console.log(`- Vulnerabilities Found: Total=${vulns.total} [Critical=${vulns.critical}, High=${vulns.high}, Moderate=${vulns.moderate}, Low=${vulns.low}]`);
...
if (vulns.total === 0) {
  reportLines.push('', '✨ **Zero vulnerabilities detected across all project packages.** Clean security bill of health.');
} else {
  reportLines.push('', '| Package | Severity | Title | Affected Range | Link |', '| --- | --- | --- | --- | --- |');
  if (auditData.vulnerabilities) {
    for (const [pkgName, vuln] of Object.entries(auditData.vulnerabilities)) {
      ...
      reportLines.push(`| \`${pkgName}\` | \`${severity}\` | ${title} | \`${range}\` | [Link](${url}) |`);
    }
  }
}
...
if (runBuild) {
  ...
  try {
    ...
    reportLines.push('', '## 🏗️ Build Verification', '', '✅ **Build Succeeded** clean compilation across client, SSR, and server bundles.');
  } catch {
    reportLines.push('', '## 🏗️ Build Verification', '', '❌ **Build Failed** compilation failed during audit check.');
    if (isStrict) {
      process.exit(1);
    }
  }
...
if (isStrict && !isClean) {
  console.error('\x1b[31m%s\x1b[0m', '[Security Audit] Strict mode enabled and Critical/High vulnerabilities exist. Failing build.');
  process.exit(1);
}

...
if (!fs.existsSync(enginePath)) {
  ...
  if (
    !content.includes("generateGstInvoicePdf") ||
    !content.includes("downloadInvoicePdf") ||
    !content.includes("09AAACS9369F1Z5")
  ) {
    console.error(
      "❌ GST invoice generator or GSTIN details missing in src/lib/pdfInvoiceEngine.ts",
    );
    passed = false;
  } else {
...
if (!fs.existsSync(edgeFuncPath)) {
  console.error(
    "❌ Supabase edge function supabase/functions/generate-invoice/index.ts not found!",
  );
  passed = false;
} else {
  ...
  if (!edgeContent.includes("PDFDocument") || !edgeContent.includes("generate-invoice")) {
    ...
    console.log(
      "✅ Supabase Edge Function confirmed (supabase/functions/generate-invoice/index.ts)",
    );
  }
}

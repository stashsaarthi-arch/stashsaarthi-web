...
- [ ] 1. **[CTO - Database]**: Audit Supabase telemetry and offline persistence queues for zero data drops.
- [ ] 2. **[CPO - Navigation Polish]**: Reorganize website navigation, deep tab synchronization, and smart explore search directory.
- [ ] 3. **[QA - Viewport Audit]**: Eliminate mobile overflow (<400px width) and enforce touch target safety.
- [ ] 4. **[CTO - Motion]**: Implement Lenis/GSAP scroll physics with GPU-accelerated layer transforms across landing page.
- [ ] 5. **[CMO - Messaging]**: Refactor hero section copy and value propositions for instant clarity.
- [ ] 6. **[CPO - UI Polish]**: Standardize card padding, glassmorphism borders, and button hover states.
- [ ] 7. **[CRO - Sticky Action]**: Add a floating, high-contrast CTA bar on mobile scroll.
- [ ] 8. **[CSO - Trust & Compliance]**: Verify laser barcode seal simulation, ₹10k micro-insurance claim terms, and TPA Sec 105 legal protections.
- [ ] 9. **[CPO - Web Audio Feedback]**: Add subtle, ultra-low-latency Web Audio API micro-haptics / clicks on interactive toggles and booking selections.
- [ ] 10. **[CEO - Auto-Research]**: CEO will inspect the whole site, generate 5 next-generation features, and log all details in `EXECUTIVE_DASHBOARD.md`.
- [ ] 11. **[QA - Performance]**: Run lighthouse audits and optimize any remaining LCP/CLS metrics to achieve 95+ score across mobile and desktop.
...
- [ ] 101. **[UI - Design Tokens / extract]**: Formalize unified OKLCH design tokens in `src/styles.css` for both Student (`--mint`, `--emerald`, `--cyan`, `--obsidian`) and Senior Host (`--amber`, `--gold`, `--obsidian`) personas.
- [ ] 102. **[UI - Color Harmony / colorize]**: Implement automated contrast checking and color harmony scales for background-to-surface layers (`--surface-1`, `--surface-2`, `--surface-elevated`).
